    ...
    for ep in (pbar):
        ...
        if True:
            ...
            if ep % 1 == 0:
                ...
                pbar.set_postfix(
                    {"res": f_trajectories.res, "f_loss": f_loss.item(), "reward": reward})
                # log metrics to wandb
                wandb.log({"res": f_trajectories.res, "f_loss": f_loss.item(), "reward": reward})
            if ep % 5 == 0:
                # checkpoint = {"gfn": gfn.state_dict()}
                dir = os.path.join(gfn_path, f"forward_gflownet_{ep}.pth")
                last_dir = os.path.join(gfn_path, f"forward_gflownet_{ep-20}.pth")
                torch.save(gfn, dir)
