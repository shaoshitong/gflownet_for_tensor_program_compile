...
def handle_file(file_idx, file):
    ...
    for line_idx, line in enumerate(lines): # 遍历每一行
        ...
        for st in steps: # 遍历每个步骤
            ...
            for i in range(len(vec), max_emb_size, 1): # 如果vec的长度小于最大嵌入大小，将0追加到vec中，直到长度等于最大嵌入大小
                vec.append(0)
        ...
        for i in range(len(step_vecs), max_seq_len, 1): # 如果step_vecs的长度小于最大序列长度，将vec复制并追加到step_vecs中，直到长度等于最大序列长度
            step_vecs.append(vec.copy())
        step_vecs = step_vecs[:args.crop_seq_len] # 对step_vecs进行切片，取出前crop_seq_len个元素

        costs = [x.value for x in outputs[line_idx].costs if isinstance(x, FloatImm)] # 从outputs中提取costs的值，只提取类型为FloatImm的元素
    ...
    for line_vec in line_vecs: # 遍历line_vecs中的每个元素
        ...
        line_vecs_new.append((step_vecs, score, min_cost)) # 将step_vecs，得分和最小代价值作为元组追加到line_vecs_new中
    line_vecs = line_vecs_new # 更新line_vecs为line_vecs_new

    return (file, file_idx, line_vecs) # 返回结果，包括文件名、文件索引、workloadkey索引、workload_key、workload参数、计算DAG的浮点操作计数和line_vecs
...
def split_dataset(file_vecs):
    ...
    test_data = []
    index = np.random.choice(len(file_vecs),int(test_ratio*len(file_vecs)),replace=False)
    print("Test Index:",index)
    np.save("test_index.npy",index)
    for file_vec_idx, file_vec in enumerate(file_vecs):

        file, file_idx, line_vecs = file_vec
        print(file_vec_idx, len(line_vecs))

        if file_vec_idx in index:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json_files_path", type=str, default='dataset_gpu/measure_records/k80-v2')
    parser.add_argument("--files_cnt", type=int, default=2000)
    ...
    test_ratio = 0.3
    files = [
        'dataset_gpu/network_info/((resnet_50,[(1,3,224,224)]),%s).task.pkl',
        'dataset_gpu/network_info/((mobilenet_v2,[(1,3,224,224)]),%s).task.pkl',
        'dataset_gpu/network_info/((resnext_50,[(1,3,224,224)]),%s).task.pkl',
        'dataset_gpu/network_info/((bert_base,[(1,128)]),%s).task.pkl',
        'dataset_gpu/network_info/((bert_tiny,[(1,128)]),%s).task.pkl'
    ]
    # for file in files:
    ...
    with open('tlp_make_dataset_str_to_idx_%s.pkl' % args.platform, 'rb') as f:
        ...
        # {'CI': 1, 'RE': 2, 'AN': 3, 'RF': 4, 'CHW': 5, 'SP': 6, 'CR': 7, 'CA': 8, 'FU': 9, 'FSP': 10, 'PR': 11}
        # {'auto_unroll_max_step$16': 1, 'auto_unroll_max_step$0': 2, 'auto_unroll_max_step$512': 3, 'auto_unroll_max_step$64': 4}
