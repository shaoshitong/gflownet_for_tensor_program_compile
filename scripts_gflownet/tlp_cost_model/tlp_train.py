...
class SegmentDataLoader:
    ...
    def _fetch_indices(self, indices):
        ...
        batch_datas_steps = self.datas_steps[indices].chunk(self.batch_size)
        for batch_data_step in batch_datas_steps:
            batch_data_step.squeeze_()
...
def load_train_datas(datasets_global):

    train_datas = np.array(datasets_global, dtype=object)
    ...
    if args.attention_class == 'gpt':
        ...
        train_dataloader = SegmentDataLoader(train_datas, args.train_size_per_gpu*n_gpu, True)
    return train_dataloader

def load_test_datas(datasets_global):

    test_datas = np.array(datasets_global, dtype=object)

    n_gpu = torch.cuda.device_count()
    if args.attention_class == 'gpt':
        test_dataloader = GPTSegmentDataLoader(test_datas, args.val_size_per_gpu*n_gpu, False)
    elif args.attention_class == 'bert':
        test_dataloader = BertSegmentDataLoader(test_datas, args.val_size_per_gpu*n_gpu, False)
    else:
        test_dataloader = SegmentDataLoader(test_datas, args.val_size_per_gpu*n_gpu, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train-dataset", type=str, default='tlp_dataset_k80_v2_2000_train_and_val.pkl')
    parser.add_argument("--test-dataset", type=str, default='tlp_dataset_k80_v2_2000_test.pkl')
    ...
    with open(args.train_dataset, 'rb') as f:
        train_datasets_global = pickle.load(f)
    with open(args.test_dataset, 'rb') as f:
        test_datasets_global = pickle.load(f)
    print('load pkl done.')
    train_datas = load_train_datas(train_datasets_global)
    test_datas = load_test_datas(test_datasets_global)
    print('create dataloader done.')
    del train_datasets_global
    del test_datasets_global
    print('load data done.')
    train(train_datas,test_datas, device=args.cuda)
