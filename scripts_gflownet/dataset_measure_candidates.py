...
from tvm.meta_schedule.runner import LocalRunner, LocalRunnerFuture, RunnerInput, RunnerResult
...
def _parse_args():
    ...
    parser.add_argument(
        "--builder_timeout_sec",
        type=int,
        default=100,#otherwise, easy to timeout
...
def measure_candidates(database:JSONDatabase, builder:LocalBuilder, runner:LocalRunner):
    ...
    with ms.Profiler() as profiler:
        ...
        with ms.Profiler.timeit("build"):
            for candi in candidates:
                builder_inputs.append(BuilderInput(candi.sch.mod, Target("llvm")))
            builder_results= builder.build(builder_inputs)
        
        runner_inputs = []
        with ms.Profiler.timeit("run"):
            for idx in range(len(builder_results)):
                if builder_results[idx].artifact_path is None:#if build failed, continue
                    #print(builder_results[idx].error_msg)    #can also change print sentence to assert sentence
                    continue
        ...
        if result.error_msg is None:
            new_database.commit_tuning_record(
                ms.database.TuningRecord(
                    ...
                    #zhangchl add msg compare to tvm's source code 
                    args_info=record.as_measure_candidate().args_info
...
def main():
    ...
    args.result_cache_dir = "./dataset_new/results"
