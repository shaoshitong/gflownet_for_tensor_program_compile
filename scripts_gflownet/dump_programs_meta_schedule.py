...
from common import load_all_tasks, get_to_measure_filename
...
def dump_program(task, size, max_retry_iter=10):
    ...
    contexts, context_weights = ms.relay_integration.extracted_tasks_to_tune_contexts(
        task,
        work_dir="./tune_tmp",
    ...
    for state in all_state_list:
        measure_inputs.append(auto_scheduler.MeasureInput(task, state)) # input = (task,state), output = (score)
