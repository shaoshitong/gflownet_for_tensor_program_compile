...
#         context = TuneContext(mod=tuning_records[0].workload.mod, target=Target(target = args.target))
...
#     parse.add_argument("--target",type=str, default="cuda -arch=sm_80")
...
def handle_json(model_dir:str):
    ...
    for workload_path in tqdm(workload_paths):
        ...
        context = TuneContext(mod=tuning_records[0].workload.mod, target=Target(args.target)).clone()
        # context = TuneContext(mod=tuning_records[0].workload.mod, target=args.target)
        features, mean_costs = extract_features(
            context, candidates, results, extractor_feature
        )
        all_dataset = add_to_group(all_dataset, features, mean_costs, shash2hex(context.mod))
...
# Target:  cuda -keys=cuda,gpu -arch=sm_80 -max_num_threads=1024 -max_shared_memory_per_block=49152 -max_threads_per_block=1024 -thread_warp_size=32
