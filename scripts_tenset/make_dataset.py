...
if __name__ == "__main__":
    ...
    if args.hold_out or args.n_task: 
        ...
        for target in args.target:
            ...
            to_be_excluded = get_hold_out_task(target, args.hold_out) # sub-tasks
            network_keys = build_network_keys() # keys (network name, {shape})
            ...
            for network_key in tqdm(network_keys):
                # Read tasks of the network
                task_info_filename = get_task_info_filename(network_key, target) # file name
                tasks, _ = pickle.load(open(task_info_filename, "rb")) # load
                for task in tasks:
